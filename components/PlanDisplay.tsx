...
const MealCard: React.FC<{ title: string; meal: Meal; icon: React.ReactNode; targetLabel: string; language: Language }> = ({ title, meal, icon, targetLabel, language }) => {
        ...
        <ul className="space-y-3">
          {meal.items.map((item, idx) => (
            <li key={`${item.foodId}-${idx}`} className="flex justify-between items-start text-sm">
              <div className="flex-1 flex items-center flex-wrap gap-1 pr-2">
                 <span className="text-emerald-500 font-bold mr-1">{item.count}x</span>
                 <span className="text-slate-300">{item.foodName}</span>
                 <a 
                    href={`https://www.youtube.com/results?search_query=${encodeURIComponent(item.foodName + ' cooking')}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="ml-2 inline-flex items-center text-slate-500 hover:text-red-500 transition-colors"
                    title={t.watchVideo}
